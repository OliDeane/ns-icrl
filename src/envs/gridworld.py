...
class Constraint():
    def __init__(self, illegal_states):
        self.illegal_states = illegal_states

    def add_constraints_on_transition_function(self, p_transition):
        for s_i in range(p_transition.shape[0]):
            for s_j in range(p_transition.shape[1]):
                if s_j in self.illegal_states:
                    for a in range(p_transition.shape[2]):
                        if p_transition[s_i, s_j, a] != 0:
                            p_transition[s_i, s_i,
                                         a] += p_transition[s_i, s_j, a]
                            p_transition[s_i, s_j, a] = 0

        return p_transition

    def constrain_state_action_transition(self, s, s_new):
        if s_new in self.illegal_states:
            return s
        return s_new


class ConstrainedGridWorld(GridWorld, Constraint):
    def __init__(self, size, illegal_states=[2, 7, 11, 13]):
        GridWorld.__init__(self, size)
        Constraint.__init__(self, illegal_states)

        self.p_transition = self.add_constraints_on_transition_function(
            self.p_transition)

    def state_index_transition(self, s, a):
        s_new = GridWorld.state_index_transition(self, s, a)

        return self.constrain_state_action_transition(s, s_new)
...
class ConstrainedIcyGridWorld(IcyGridWorld, Constraint):
    def __init__(self, size, illegal_states=[2, 7, 11, 13]):
        IcyGridWorld.__init__(self, size)
        Constraint.__init__(self, illegal_states)
    ...
    def state_index_transition(self, s, a):
        s_new = IcyGridWorld.state_index_transition(self, s, a)
