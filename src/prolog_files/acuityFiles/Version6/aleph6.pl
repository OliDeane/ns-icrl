...
%                                                                         %
% A L E P H                                                               %
% Version 6      (last modified: Sun May 21 12:00:00 UTC 2017)            %
%                                                                         %
% This is the source for Aleph written and maintained                     %
% by Ashwin Srinivasan (ashwin.srinivasan@wolfson.oxon.org)               %
%                                                                         %
%                                                                         %
...
init(yap):-
	...
	assert_static((aleph_inf(Inf):- Inf is inf)),
	% assert_static((aleph_wait_for_input(Stream,_,_):- wait_for_input([Stream],_,0))).
	assert_static((aleph_wait_for_input(_,_,_):- true)).
...
init(swi):-
	...
	(predicate_property(delete_file(_),built_in) -> true;
		assert_static(delete_file(_))),
		% assert((aleph_wait_for_input(Stream,_,_):- wait_for_input([Stream],_,infinite))).
	assert((aleph_wait_for_input(_,_,_):- true)).
...
aleph_version(6).
aleph_version_date('Sun Jan 1 12:00:00 UTC 2017').
...
:- dynamic '$pycuity_search_store'/1. % Thos stores all clauses search by aleph during single-clause induction
:- dynamic '$pycuity_constraint_violations'/3. % This stores all clauses searched that violate user-defined constraints
:- dynamic '$pycuity_user_trace'/2. % This stores a trace of user actions
...
rm_nreduce(Last,N):-
	...
	length(Lits,N),
	p1_message(1,'negative-based removal'), p_message(1,N/Last).
rm_nreduce(_,0).
...
rm_moderepeats(Last,N):-
	...
	get_marked(1,Last,Lits),
	length(Lits,N),
	p1_message(1,'repeated literals'), p_message(1,N/Last),
	remove_lits(Lits).
rm_moderepeats(_,0).
...
rm_symmetric(Last,N):-
	...
	get_marked(1,Last,Lits),
	length(Lits,N),
	p1_message(1,'symmetric literals'), p_message(1,N/Last),
	remove_lits(Lits).
rm_symmetric(_,0).
...
rm_commutative(_,_):-
	...
	p1_message(1,'checking commutative literals'),
	p_message(1,Name/Arity),
...
rm_commutative(Last,N):-
	...
	get_marked(1,Last,Lits),
	length(Lits,N),
	p1_message(1,'commutative literals'), p_message(1,N/Last),
	remove_lits(Lits).
rm_commutative(_,0).
...
rm_uselesslits(Last,N):-
	...
	length(Lits,N),
	p1_message(1,'useless literals'), p_message(1,N/Last),
	remove_lits(Lits).
rm_uselesslits(_,0).
...
rm_redundant(Last,N):-
	...
	get_marked(1,Last,Lits),
	length(Lits,N),
	p1_message(1,'redundant literals'), p_message(1,N/Last),
	remove_lits(Lits).
...
get_nsuccesses(Literal,N,M):-
	...
	(((Succ1 >= N);(botsize_limit_reached)) -> !; true).

botsize_limit_reached:-
	'$aleph_local'(botsize,B0),
	setting(max_botsize,BMax),
	B0 >= BMax, !.
...
update_atoms(Atom,M):-
	...
	retract('$aleph_local'(botsize,B0)),
	B1 is B0 + 1,
	assertz('$aleph_local'(botsize,B1)).
...
get_search_settings(S):-
	...
	setting(openlist,OSize), arg(25,S,OSize),
        ...
        ('$aleph_sat'(hovars,HOVars) -> arg(28,S,HOVars); arg(28,S,_HOVars)),
	...
	(setting(recursion,_Recursion) -> true; _Recursion = false),
	...
	setting(clausetype,CType), arg(48,S,CType),
	(setting(streaming,Streaming) -> arg(49,S,Streaming); arg(49,S,false)).
...
% get output vars by matching a mode
get_ovars_from_mode(Loc,Lit,OVars):-
	functor(Lit,Name,Arity),
        functor(Mode,Name,Arity),
	(Loc=head ->
		'$aleph_global'(modeh,modeh(_,Mode));
		'$aleph_global'(modeb,modeb(_,Mode))),
        split_args(Mode,Mode,_,O,_),
	get_args(Lit,O,[],AllVars),
	sort(AllVars,OVars).
...
% get input vars by matching a mode
get_ivars_from_mode(Loc,Lit,IVars):-
	functor(Lit,Name,Arity),
        functor(Mode,Name,Arity),
	(Loc=head ->
		'$aleph_global'(modeh,modeh(_,Mode));
		'$aleph_global'(modeb,modeb(_,Mode))),
        split_args(Mode,Mode,I,_,_),
	get_args(Lit,I,[],AllVars),
	sort(AllVars,IVars).
...
get_sibgain(S,LVars,LitNum,Desc,Node,LastSib,Last,Best,Path,C,TV,L,Min,Pos,Neg,OVars,E,LBest,LNode):-
	arg(34,S,Proof),
        arg(23,S,Lazy),
	(Proof = user ->
		'$aleph_search_node'(Node,Sib1,_,_,_,_,_,_),
		\+(aleph_member1(Sib1,Desc)),
		arg(16,S,PSize), arg(24,S,NSize),
		PC = [1-PSize], NC = [1-NSize],
		Flag = upper,
		...
		(intersects(SibIVars,LVars) -> Flag = upper;
			...
			(intersects(SibOVars,LVars) -> Flag = upper;
				Flag = exact))),
...
get_theory_gain(S,Last,BestSoFar,T0,Pos,Neg,P,N,Best1,NewLast):-
	...
	arg(3,S,RefineOp),
	(RefineOp = rls -> rls_refine(theories,T0,T1);
		(RefineOp = user ->
			refine(T0,T1);
			aleph_delete(_,T0,T1))),
	arg(23,S,LazyPreds),
	(LazyPreds = [] -> Theory = T1;
...
get_theory_gain1(S,Theory,Last,Best,Pos,Neg,P,N,Best1):-
        (false -> p_message(1,'constraint violation'),
                Contradiction = true;
                Contradiction = false),
	Contradiction = false,
        Node1 is Last + 1,
	...
	(Proof = user ->
		Clauses = Theory;
		Clauses = (X:-X)),
        prove(Depth/Time/Proof,pos,Clauses,Pos,PCvr,TP),
        prove(Depth/Time/Proof,neg,Clauses,Neg,NCvr,FP),
	arg(4,S,_/Evalfn),
	Correct is TP + (N - FP),
...
get_gain1(S,_,C,CL,_,Last,Best,Path,_,Pos,Neg,_,E,Best):-
	...
	(VSearch = true ->
		...
		assertz('$aleph_search_node'(Node1,C));
		true).
get_gain1(S,_,Clause,_,_,_,Best,_,_,_,_,_,_,Best):-
        ...
        p_message(1,'in prune cache').
get_gain1(S,Flag,C,CL,EMin/EL,Last,Best/Node,Path,L1,Pos,Neg,OVars,E,Best1):-
	split_clause(C,Head,Body),
	arg(22,S,Search),
        ((Search \== ic, false) -> p_message(1,'constraint violation'),
			% asserta('$pycuity_constraint_violations'(C)),
                Contradiction = true;
                Contradiction = false),
        Node1 is Last + 1,
        arg(8,S,Caching),
...
% at present only used by add_bottom
clause_ok(Clause,_):-
	setting(clausetype,CType),
	CType \= any,
	\+ clausetype_ok(CType,Clause), !, fail.
clause_ok(Clause,_):-
...
clause_ok(_,Label):-
	\+ var(Label),
	extract_pos_count(Label,P),
	extract_neg_count(Label,N),
	Acc is P/(P+N),
	setting(noise,Noise),
...
refinement_ok(_,Entry):-
	...
	p_message(1,'redundant refinement'),
	fail.
refinement_ok(_,_).
...
split_ok(_,Clause,Lit):-
	...
	p_message(1,'redundant literal'), nl,
	fail.
split_ok(_,_,_).
...
theory_lang_ok([],_).
theory_lang_ok([C|T],Lang):-
	(C = _-[_,_,_,Clause] -> true; Clause = C),
        lang_ok(Clause,Lang),
        theory_lang_ok(T,Lang).

theory_newvars_ok([],_).
theory_newvars_ok([C|T],NewV):-
	(C = _-[_,_,_,Clause] -> true; Clause = C),
...
lang_ok((Head:-Body),N):-
	...
	(lang_ok(N,Head,Body) -> true;
		p_message(1,'outside language bound'),
		fail).
...
clausetype_ok(Type,Clause):-
	setting(construct_bottom,Bottom),
	clausetype_ok(Type,Bottom,Clause).

clausetype_ok(any,_,_):- !.
clausetype_ok(elementary,Bottom,Clause):-
	elementary_clause_ok(Bottom,Clause), !.
clausetype_ok(simple,Bottom,Clause):-
	simple_clause_ok(Bottom,Clause), !.
clausetype_ok(rsd,Bottom,Clause):-
	rsd_clause_ok(Bottom,Clause), !.
clausetype_ok(independent,Bottom,Clause):-
	independent_clause_ok(Bottom,Clause), !.
clausetype_ok(_,_,_):-
	p_message('clausetype violation'), !, fail.


simple_clause_ok(Bottom,Clause):-
	(Bottom \= false ->
		once(get_aleph_clause(Clause,AlephClause)),
		copy_term(AlephClause,AClause),
		match_bot_lits(AClause,[],[Head|Body]),
		sink_literals(Head,Body,Sinks),
		length(Sinks,N);
		source_sink_literals_from_modes(Clause,_,Sinks),
		length(Sinks,N)),
	N =< 1.

independent_clause_ok(_,(Head:-Body)):-
...
rsd_clause_ok(Bottom,Clause):-
	independent_clause_ok(Bottom,Clause),
	(Bottom \= false ->
		source_sink_literals(Clause,Body,Sources,Sinks),
		length(Sinks,N),
		N >= 1,
		connected_source_literals(Sources,Body);
		source_sink_literals_from_modes(Clause,Sources,Sinks),
		length(Sinks,N),
		N >= 1,
		mode_connected_source_literals(Sources, _Body)).
...
sink_literals(Head,BodyLits,Sinks):-
	findall(Sink,sink_literal(Head,BodyLits,Sink),Sinks).

sink_literal(Head,BodyLits,Sink):-
	aleph_delete(Sink,BodyLits,Left),
	\+ source_literal(Head,Left,Sink).

source_literal(Lit,[],Lit):- !.
source_literal(_,Body,Lit):-
	get_ovars1(false,Lit,OVars),
	OVars \= [],
	get_ivars(Body,false,[],IVars),
	intersects(OVars,IVars).

source_literal_from_modes(Lit,Lits):-
	% Lits \= [],
	get_ovars_from_mode(body,Lit,OVars),
	aleph_member(Lit1,Lits),
	get_ivars_from_mode(body,Lit1,IVars),
	aleph_member(OVar,OVars),
	aleph_member(IVar,IVars),
	OVar == IVar, !.

% split body literals in a clause into those that have no output variables
% and those that have at least one output variable.
% The former have been called as "property literals" and the latter
% "structural predicates" in
% Flach and Lachiche (1999) "1BC: a first-order
% Bayesian Classifier", Proc 9th Intnl. Conf. ILP, LNAI-1634
% and are somewhat related to the source and sink literals of McCreath
source_sink_literals(Clause,Body,Sources,Sinks):-
	once(get_aleph_clause(Clause,AlephClause)),
	copy_term(AlephClause,AClause),
	match_bot_lits(AClause,[],[_|Body]),
	get_source_sink(Body,Sources,Sinks).


source_sink_literals_from_modes((Head:-true),[Head],[]):- !.
source_sink_literals_from_modes(Clause,[Head|Sources],Sinks):-
	clause_to_list(Clause,[Head|Body]),
	get_source_sink_from_modes(Body,Sources,Sinks).

get_source_sink([],[],[]).
get_source_sink([Lit|LitNums],Sources,[Lit|Sinks]):-
	'$aleph_sat_litinfo'(Lit,_,_,_,[],[]), !,
	get_source_sink(LitNums,Sources,Sinks).
get_source_sink([Lit|LitNums],[Lit|Sources],Sinks):-
	get_source_sink(LitNums,Sources,Sinks).


get_source_sink_from_modes([],[],[]).
get_source_sink_from_modes([Lit|Lits],[Lit|Sources],Sinks):-
	\+(\+(source_literal_from_modes(Lit,Lits))), !,
	get_source_sink_from_modes(Lits,Sources,Sinks).
get_source_sink_from_modes([Lit|Lits],Sources,[Lit|Sinks]):-
	get_source_sink_from_modes(Lits,Sources,Sinks).



connected_source_literals([],_).
connected_source_literals([Lit|Lits],Body):-
	aleph_delete(Lit,Body,BodyLeft),
	get_ivars(BodyLeft,false,[],IVars),
	get_ovars1(false,Lit,OVars),
	OVars \= [],
	aleph_subset1(OVars,IVars),
	connected_source_literals(Lits,Body).

newvars_ok((Head:-Body),N):-
	!,
	(newvars_ok(N,Head,Body) -> true;
		p_message(1,'outside newvars bound'),
		fail).
...
lang_ok1([],_).
lang_ok1([P/A|Preds],NList):-
        length(Preds,N0),
        aleph_delete_all(P/A,Preds,Preds1),
        length(Preds1,N1),
	(aleph_member1(N-P/A,NList)-> true;
		number(inf,N)),
        PredOccurs is N0 - N1 + 1,
	PredOccurs =< N,
	lang_ok1(Preds1,NList).
...
prove_pos(S,_,_,_,_,_,[Pos],[Pos],1):-
	arg(49,S,Streaming),
	Streaming = true, !.
...
prove_neg(S,_,_,_,_,_,[Neg],[Neg],1):-
	arg(49,S,Streaming),
	Streaming = true, !.
...
update_best(S,_,_,_,Best,[P,_,_,F|_]/_,Best):-
        arg(13,S,MinPos),
        arg(19,S,MinScore),
	(P < MinPos;  number(-inf,F); F < MinScore), !.
...
update_best(S,Clause,_,_,Best,_,Best):-
	arg(48,S,CType),
	\+ clausetype_ok(CType,Clause), !.
update_best(S,Clause,PCover,NCover,Label/_,Label1/Node1,Label1/Node1):-
	...
	retractall('$aleph_search'(selected,_)),
		...
		retractall('$aleph_search'(best,_)),
                ...
                true),
	arg(44,S,ConstructFeatures),
	update_good(ConstructFeatures,Label1,Clause),
...
update_good(ConstructFeatures,Label,Clause):-
	setting(good,true),
	setting(clausetype,CType),
	clausetype_ok(CType,Clause), !,
	Label = [_,_,L|_],
	setting(check_good,Flag),
	('$aleph_sat'(example,example(Num,Type)) -> true;
		Num = 0, Type = pos),
	update_good(ConstructFeatures,Flag,L,Label,Num,Type,Clause).
update_good(_,_,_).

update_good(_,_,_,_,_,_,_):-
	setting(goodfile,_), !.
update_good(_,true,L,Label,_,_,Clause):-
	exists_good(L,Label,Clause), !.
update_good(ConstructFeatures,_,L,Label,Num,Type,Clause):-
	...
	('$aleph_search'(max_good,MaxGood) ->
		Good1 =< MaxGood;
		true),
	assertz('$aleph_good'(L,Good1,Label,Num,Type,Clause)),
	(ConstructFeatures = true -> gen_feature_from_good_clause(Good1); true),
	assertz('$aleph_search'(last_good,Good1)), !.
update_good(_,_,_,_,_,_,_):-
	'$aleph_search'(max_good,MaxGood),
	assertz('$aleph_search'(last_good,MaxGood)), !.

exists_good(L,_,Clause):-
	clause_to_list(Clause,ClauseList),
	setting(check_good_dist,Dmin),
	'$aleph_good'(L,_,_,_,_,Clause1),
	clause_to_list(Clause1,Clause1List),
	match_dist(ClauseList,Clause1List,D),
	D =< Dmin, !.

match_dist([_|B1],[_|B2],D):-
        length(B1,L1),
        length(B2,L2),
        matches(B1,B2,0,N),
        D is 1 - N/(L1+L2-N).
...
matches([],_,N,N).
matches([Goal|Goals],L,N0,N):-
        delete_variant(Goal,L,L0),
        N1 is N0 + 1,
        matches(Goals,L0,N1,N).
matches([_|Goals],L,N0,N):-
        matches(Goals,L,N0,N).

delete_variant(Goal,[G|Goals],Goals):-
        alpha_variant(Goal,G),  !.
delete_variant(Goal,[G|Goals],[G|Left]):-
        delete_variant(Goal,Goals,Left).
...
prune_open(S,_,_):-
	...
	OSize =\= Inf,
		...
		arg(6,S,Verbose),
                (Verbose < 1 ->
			true;
			p1_message(1,'non-admissible removal'),
			p_message(1,NodeNum))),
        fail.
prune_open(S,_,_):-
        arg(2,S,Explore),
...
prove2([Current-Finish|Intervals],Depth/Time/Proof,Type,(Head:-Body),InCount,Sofar,OutCount) :-
	'$aleph_example'(Current,Type,Example),
...
find_tree(Type):-
	...
	p1_message(1,'construction time'), p_message(1,Time).
...
can_split(S,Type,Predict,Leaf,Left,Right):-
	...
	p1_message(1,'expected cost reduction'), p_message(1,Gain).
...
get_best_subtree(S,Type,Predict,[Clause,Cost,N],Examples,Gain,Left,Right):-
	...
	(Interactive = false ->
		...
		p1_message(1,'expected cost reduction'),
		p_message(1,CostReduction),
...
create_copy(X,Y,OldT,NewT,Ab,Num,Last):-
	'$aleph_example'(X,OldT,Example),
	Example =.. [_|Args],
	NewExample =.. [Ab|Args],
	Num1 is Num + 1,
	aleph_writeq('$aleph_example'(Num1,NewT,NewExample)), write('.'), nl,
	X1 is X + 1,
...
redundant(Lit,Lits,[Head|Body]):-
	...
	aleph_member(Lit1,Body),
	Lit = Lit1,
	aleph_subsumes(Lits,Rest1).


aleph_subsumes(Lits, Lits1) :- subtle_subsumes(Lits, Lits1).

/*
   This is a very well known device, but is formally explained in
   "A simple framework for theta-subsumption testing in Prolog"
   by Hendrik Blockeel and Svetlana Valevich. SUBTLE is the acronym
   provided in that paper.

   Note: SWI "subset" and Aleph "aleph_subset1" both have cuts!
*/
subtle_subsumes(C,D) :-
    \+ \+ (copy_term(D, D2), numbervars(D2, 0, _), subtle_subset(C, D2)).

subtle_subset([], _).
subtle_subset([A|B], D) :- subtle_member(A, D), subtle_subset(B,D).

%   From SWI-Prolog lists library. It is more efficient and determnistic
%   on the last element.
%   @author Gertjan van Noord

subtle_member(El, [H|T]) :-
    subtle_member_(T, El, H).

subtle_member_(_, El, El).
subtle_member_([H|T], El, _) :-
    subtle_member_(T, El, H).
...
% reduce an existing theory
treduce:-
	setting(search,Search),
	catch(treduce(Search),abort,reinstate_values), !.
...
reduce(ibs):-
	...
	p1_message(1,'ibs beam width'), p_message(1,OldOpen),
	find_clause(bf),
	'$aleph_search'(current,current(_,Nodes0,[PC,NC|_]/_)),
	N is NC + PC,
	estimate_error_rate(Nodes0,0.5,N,NC,NewR),
	p1_message(1,'ibs estimated error'), p_message(1,NewR),
	...
	p1_message(1,'ibs clauses constructed'), p_message(1,Nodes),
	p1_message('ibs search time'), p_message(Time),
	p_message(1,'ibs best clause'),
	pp_dclause(RClause),
...
reduce(id):-
	...
	p1_message(1,'id clauselength setting'), p_message(1,OldCLen),
	find_clause(df),
	...
	add_hyp(BestLabel,RClause,PCover,NCover),
	p1_message(1,'id clauses constructed'), p_message(1,Nodes),
	p1_message(1,'id search time'), p_message(1,Time),
	p_message(1,'id best clause'),
	pp_dclause(RClause),
	show_stats(Evalfn,BestLabel),
	record_search_stats(RClause,Nodes,Time),
	noset(best),
...
reduce(ils):-
	...
	stopwatch(Start),
	repeat,
	setting(language,[OldLang-_]),
	p1_message(1,'ils language setting'), p_message(1,OldLang),
	find_clause(bf),
	'$aleph_search'(current,current(_,Nodes0,_)),
	...
	add_hyp(BestLabel,RClause,PCover,NCover),
	p1_message(1,'ils clauses constructed'), p_message(1,Nodes),
	p1_message(1,'ils search time'), p_message(1,Time),
	p_message(1,'ils best clause'),
	pp_dclause(RClause),
	show_stats(Evalfn,BestLabel),
	record_search_stats(RClause,Nodes,Time),
	noset(best),
...
reduce(rls):-
	...
	p1_message(1,'rls nodes constructed'), p_message(1,Nodes),
	p1_message(1,'rls search time'), p_message(1,Time),
	p_message(1,'rls best result'),
	pp_dclause(RBest),
	setting(evalfn,Evalfn),
	show_stats(Evalfn,BestLabel),
...
reduce(scs):-
	...
	(setting(scs_type,informed)->
		(setting(clauselength_distribution,_D) -> true;
			...
			p1_message(1,'using clauselength distribution'),
			p_message(1,D),
	...
	p1_message(1,'scs nodes constructed'), p_message(1,Nodes),
	p1_message(1,'scs search time'), p_message(1,Time),
	p_message(1,'scs best result'),
	pp_dclause(RBest),
	setting(evalfn,Evalfn),
	show_stats(Evalfn,BestLabel),
	record_search_stats(RBest,Nodes,Time),
...
% find_theory(Search) searches the space of theories
find_theory(rls):-
	!,
	...
	setting(evalfn,Evalfn),
	Theory = [1-[0,0,[],false]],
        complete_label(Evalfn,Theory,[P0,N0,1],Label),
	asserta('$aleph_search'(rls_selected,selected(Label,Theory,
						PosSet,NegSet))),
	...
	p1_message(1,'restart'), p_message(1,R),
	find_theory1(Theory),
	'$aleph_search'(current,current(_,Nodes0,_)),
	...
	add_hyp(BestLabel,RBest,PCover,NCover),
	p1_message(1,'nodes constructed'), p_message(1,Nodes),
	p1_message(1,'search time'), p_message(1,Time),
	p_message(1,'best theory'),
	pp_dclauses(RBest),
	show_stats(Evalfn,BestLabel),
	record_search_stats(RBest,Nodes,Time),
	noset(best),
...
find_theory1(T0):-
	...
	(T0 = [L-[_,_,_,_]] -> true;
		length(T0,L)),
        complete_label(Evalfn,T0,[P,N,L],Label),
	set(best,Label),
	asserta('$aleph_search'(selected,selected(Label,T0,Pos,Neg))),
	get_theory_gain(S,0,Label/0,T0,Pos,Neg,P,N,NextBest,Last),
	...
	p1_message(1,'theories constructed'), p_message(1,Nodes),
	p1_message(1,'search time'), p_message(1,Time),
	p_message(1,'best theory'),
	pp_dclauses(RTheory),
	show_stats(Evalfn,BestLabel),
...
induce:-
	setting(induce,Induce),
	induce(Induce).

% compatibility predicates for previous versions
induce_max:-
	induce(max).
induce_cover:-
	induce(cover).
...
% induce(greedy): non-interactive theory construction
% constructs theories 1 clause at a time
...
induce(greedy):-
	...
	assertz('$aleph_global'(other_stats,search_time_taken(Time))),
	show_total_stats,
	record_total_stats, !.
induce(greedy).

% induce(max): construct theories 1 clause at a time
...
% induce(cover): construct theories 1 clause at a time
% does not perform greedy cover removal after each clause found
induce(cover):-
	clean_up,
	retractall('$aleph_global'(search_stats,search_stats(_,_))),
...
% induce(stream): simple on-line learning
% here, just a basic repeat-fail loop that waits for input
% on a pre-defined data-sream (set by assigning the variable
% stream) and calls a user-defined predicate aleph_stream/1.
% It is expected that aleph_stream/1 would define the
% procedure for processing the example read in from the stream.
% Stream processing stops when an end-of-file is encountered
% in the stream.
induce(stream):-
	setting(stream,File),
	set(streaming,true),
	...
	stopwatch(StartClock),
	repeat,
	aleph_wait_for_input(Stream,_,_),
	read(Stream,Example),
	clean_up_examples,
	(Example = end_of_file -> true;
		record_example(check,_,Example,_),
		(setting(stream_count,N) -> N1 is N + 1; N1 is 1),
		set(stream_count,N1),
		show_stream_count(N1),
		once(aleph_stream(Example)),
		fail),
	!,
	stopwatch(StopClock),
	Time is StopClock - StartClock,
	assertz('$aleph_global'(other_stats,stream_time_taken(Time))),
	noset(streaming),
	noset(stream_count),
	close(Stream),
	show_total_stats,
...
show_stream_count(N1):-
	...
	% open(fred,append,S),
	% set_output(S),
        p1_message('streaming example: '), p_message(N1).
	% flush_output(S), close(S),
	% set_output(user_output).
show_stream_count(_).


%  induce(theory)/induce: does theory-level search
...
% induce(constraints): search for logical constraints that
...
% induce(modes): search for an acceptable set of mode declarations
induce(modes):-
	clean_up,
...
% induce(features): search for interesting boolean features
...
% A variant attempts to construct features by performing no
% combinatorial search. That is, step (b) is eliminated by
% enumerating all clauses in the search-space of the example
% selected and storing them in a prefix-tree (that is, a trie). At
% the end, the good clauses in the prefix tree are enumerated.
induce(features):-
	setting(feature_construction,theory), !,
        ('$aleph_global'(rules,rules(_)) -> true;
		induce(greedy)),
	get_theory_features.
induce(features):-
	clean_up,
	store_values([good,nodes,check_good,updateback,
		construct_features,samplesize,greedy,explore,
		lazy_on_contradiction,search,scs_sample]),
	set(good,true),
	set(check_good,true),
	set(updateback,false),
	set(construct_features,true),
	set(lazy_on_contradiction,true),
	setting(feature_construction,FConstruct),
	setting(max_features,FMax),
	(FConstruct = exhaustive -> set(explore,true);
		(FConstruct = random ->
			set(search,scs),
			set(scs_sample,FMax);
			true)),
	...
	repeat,
        gen_sample(pos,1),
	get_example_class(Class),
	set_feature_search_restrictions(MaxNodes,FMax),
	('$aleph_search'(last_good,LastGood) -> true; LastGood = 0),
	retractall('$aleph_global'(besthyp,besthyp(_,_,_,_,_))),
	number(-inf,MInf),
        asserta('$aleph_global'(besthyp,besthyp([MInf,0,1,MInf],0,(false),[],[]))),
	construct_features(Class),
	show_atoms_left,
	record_atoms_left,
	('$aleph_search'(last_good,LastGood1) -> true; LastGood1 = 0),
	NewFeatures is LastGood1 - LastGood,
	update_feature_count(Class,NewFeatures),
        '$aleph_global'(atoms_left,atoms_left(pos,[])), !,
	% gen_features,
        stopwatch(StopClock),
        Time is StopClock - StartClock,
	show(features),
	assertz('$aleph_global'(other_stats,feature_construction_time(Time))),
	(setting(streaming,true) ->
			setting(stream_count,S),
			p1_message('features constructed from stream example'),
			p_message(S),
			asserta('$aleph_global'(other_stats,features_from_stream_example(S)));
			true),
        ...
        reinstate_values([good,nodes,check_good,updateback,
			construct_features,samplesize,greedy,explore,
			lazy_on_contradiction,search,scs_sample]), !.
induce(features).

construct_features(Class):-
        setting(feature_focus,FClass), !,
	(FClass = Class  ->
		construct_new_features(Class);
		true),
	(setting(streaming,true) ->
		record_model_features;
		true).
construct_features(Class):-
	construct_new_features(Class),
	(setting(streaming,true) ->
...
construct_new_features(Class):-
	setting(feature_construction,user), !,
	get_user_features(Class),
	addhyp.
construct_new_features(_):-
	get_besthyp(false),
	addhyp.

get_user_features(Class):-
	('$aleph_search'(max_features,N) -> true; N = 1),
	(setting(dependent,ClassArg) -> true; ClassArg = 0),
	setting(evalfn,Evalfn),
	get_user_features(N,Class,ClassArg,Evalfn,BestFeature),
	add_bestfeature(BestFeature),
	retract('$aleph_global'(example_selected,
				example_selected(pos,Num))),
	rm_seeds(pos,[Num-Num]).

get_user_features(N,Class,ClassArg,EvalFn,Best):-
	retractall('$aleph_local'(featurecounter,_)),
	retractall('$aleph_local'(featurebest,_)),
	'$aleph_global'(example_selected,
				example_selected(pos,Num)),
	sat(Num),
	asserta('$aleph_local'(featurecounter,0)),
	get_start_feature(EvalFn,Best0),
	asserta('$aleph_local'(featurebest,Best0)),
	repeat,
	aleph_feature(Feature),
	retract('$aleph_local'(featurecounter,N0)),
	N1 is N0 + 1,
	asserta('$aleph_local'(featurecounter,N1)),
	(N1 > N -> !,
		retractall('$aleph_local'(featurecounter,_)),
		retract('$aleph_local'(featurebest,Best));
		update_good_user_feature(Feature,Class,ClassArg,EvalFn),
		fail).
%  get_user_features(_,_,_,_,0-[]).
get_user_features(_,_,_,_,Best):-
	retract('$aleph_local'(featurebest,Best)), !.
get_user_features(_,_,_,_,0-[]).

get_start_feature(Evalfn,F0-B0):-
	get_start_label(Evalfn,Label),
	('$aleph_sat'(example,example(Num,Type)) ->
		'$aleph_example'(Num,Type,Example),
		...
		PC = [],
		NC = []),
	evalfn(Evalfn,Label,F0),
	B0 = [Label,PC,NC,Feature].

update_good_user_feature(Feature,Class,ClassArg,EvalFn):-
	Feature = (Head:-_),
	(ClassArg = 0 -> true; arg(ClassArg,Head,Class)),
	label_create(Feature,ExtendedLabel),
	assemble_label(ExtendedLabel,Label),
	clause_ok(Feature,Label),
	complete_label(EvalFn,Feature,Label,Label1),
	update_good(true,Label1,Feature),
	extract_eval(Label1,F1),
	retract('$aleph_local'(featurebest,F0-B0)),
	(F1 > F0 ->
		F = F1,
		label_cover(pos,ExtendedLabel,PC),
		label_cover(neg,ExtendedLabel,NC),
		B = [Label,PC,NC,Feature];
		F = F0,
		B = B0),
	asserta('$aleph_local'(featurebest,F-B)), !.

get_user_features(0,_,_,_,Best,Best):- !.
get_user_features(N,Class,ClassArg,EvalFn,F0-B0,Best):-
	aleph_feature(Feature),
	Feature = (Head:-_),
	arg(ClassArg,Head,Class),
	label_create(Feature,ExtendedLabel),
	assemble_label(ExtendedLabel,Label),
	clause_ok(Feature,Label), !,
	complete_label(EvalFn,Feature,Label,Label1),
	...
	(F1 > F0 ->
		...
		B = B0),
	N0 is N - 1,
	get_user_features(N0,Class,ClassArg,EvalFn,F-B,Best).
get_user_features(N,Class,ClassArg,EvalFn,F0-B0,Best):-
	N0 is N - 1,
	get_user_features(N0,Class,ClassArg,EvalFn,F0-B0,Best).

add_bestfeature(_-[]):- !.
add_bestfeature(_-[Label,PC,NC,Feature]):-
	add_hyp(Label,Feature,PC,NC).

get_theory_features:-
        '$aleph_global'(rules,rules(L)),
        aleph_reverse(L,L1),
        aleph_member(ClauseNum,L1),
        '$aleph_global'(theory,theory(ClauseNum, Label/_,Clause,_,_)),
	gen_feature(Clause,Label),
	fail.
get_theory_features.

% induce(model): construct a model using boolean features known to Aleph
% Usually, the boolean features would have been constructed beforehand
% using induce_features/0.
% The model-construction technique is set by the parameter model_type.
% Currently model_type can only be one of two values: winnow1, winnow2, or user.
% If model_type = winnow1 or winnow2, then Aleph constructs a linear-threshold
% classifier along the lines of the Winnow algorithm described in
% N. Littlestone (1988) "Learning Quickly When Irrelevant Attributes Abound: A New
% Linear Threshold Algorithm", Machine Learning (2):285--318. The Winnow2
% algorithm promotes or demotes weights using a multiplicative factor alpha. Winnow1,
% demotes weights to 0, but promotes weights by multiplying by alpha.
% This alpha factor is set using winnow_rate, the default value of which is 2 (that is
% weights will be doubled or halved after each iteration in Winnow2 or doubled or zeroed
% after each iteration in Winnow1).
% The user can set a threshold using winnow_threshold. If the weighted sum of
% features is below this number then the class is predicted as negative.
% If no threshold is set, then it is simply taken to be N, the number of features
% known to Aleph.
% It has been shown that the Winnow algorithm has a mistake-bound that is logarithmic
% in the number of features N
% If model_type = user, then Aleph expects the definition of aleph_model/1.
%
induce(model):-
	clean_up,
	last_feature(N),
	N > 0, !,
	setting(model_type,Type),
	construct_model(Type),
	show_total_stats,
	record_total_stats.
induce(model).


construct_model(user):-
	!,
	aleph_model(_),
	show(model).
construct_model(Model):-
	(Model = winnow1; Model = winnow2),
	set_winnow_rates(Model),
	construct_model(winnow).

set_winnow_rates(_):-
	setting(winnow_promotion_rate,_),
	setting(winnow_demotion_rate,_), !.
set_winnow_rates(Winnow):-
	setting(winnow_rate,Alpha), !,
	set(winnow_promotion_rate,Alpha),
	(Winnow = winnow1 -> set(winnow_demotion_rate,0);
		set(winnow_demotion_rate,1/Alpha)).

construct_model(winnow):-
	(setting(streaming,true) -> true;
		retractall('$aleph_global'(other_stats,_))),
	record_winnow_pclause,
	record_settings,
	stopwatch(StartClock),
	find_model(winnow),
        stopwatch(StopClock),
...
record_winnow_pclause:-
	retractall('$aleph_search'(pclause,_)),
	(setting(classes,Classes) -> Classes = [Pos,Neg]; Pos = pos, Neg = neg),
	Body = (
		feature_vector(Example,V),
		(setting(winnow_threshold,Threshold) -> true;
			'$aleph_feature_count'(Threshold)),
		(weighted_sum_threshold(V,0,Threshold) -> Pred = Pos; Pred = Neg),
		(setting(dependent,ClassArg) -> arg(ClassArg,Example,Pred); true)),
	assertz('$aleph_search'(pclause,pclause(Example,Body))).


find_model(winnow):-
	examples(pos,PL),
	examples(neg,NL),
	'$aleph_feature_count'(LastFeature),
        (setting(winnow_threshold,Threshold) -> true;
		Threshold = LastFeature),
	setting(winnow_promotion_rate,P),
	setting(winnow_demotion_rate,D),
	winnow_update(PL,pos,P,Threshold),
	winnow_update(NL,neg,D,Threshold).

record_model_features:-
	'$aleph_global'(other_stats,model_features(I,_)), !,
	'$aleph_feature_count'(LastFeature),
	I1 is I + 1,
	asserta('$aleph_global'(other_stats,model_features(I1,LastFeature))).
record_model_features:-
	'$aleph_feature_count'(LastFeature),
	asserta('$aleph_global'(other_stats,model_features(1,LastFeature))).

winnow_update([],_,_,_):- !.
winnow_update([Example|Examples],Actual,Rate,Threshold):-
	feature_vector(Example,Actual,FeatureVector),
	weighted_sum(FeatureVector,0,Sum),
	(Sum >= Threshold -> Pred = pos; Pred = neg),
	(Actual = Pred -> true;
		update_winnow_mistakes,
		winnow_multiply_weight(FeatureVector,Rate)),
	winnow_update(Examples,Actual,Rate,Threshold).


update_winnow_mistakes:-
	(retract('$aleph_global'(other_stats,model_mistakes(M)))-> M1 is M + 1; M1 is 1),
	asserta('$aleph_global'(other_stats,model_mistakes(M1))).

weighted_sum([],S,S).
weighted_sum([F-X|Rest],S0,Sum):-
	feature_weight(F,W),
	S1 is W*X + S0,
	weighted_sum(Rest,S1,Sum), !.

weighted_sum_threshold([],S0,Threshold):- S0 >= Threshold, !.
weighted_sum_threshold([F-X|Rest],S0,Threshold):-
	feature_weight(F,W),
	S1 is W*X + S0,
	(S1 >= Threshold -> true;
		weighted_sum_threshold(Rest,S1,Threshold)), !.


update_feature_weight(F,M):-
	retract('$aleph_feature_weight'(F,W0)), !,
	W is float(W0*M),
	asserta('$aleph_feature_weight'(F,W)).
update_feature_weight(F,M):-
	W is float(M),
	asserta('$aleph_feature_weight'(F,W)).


winnow_multiply_weight([],_):- !.
winnow_multiply_weight([F-1|Rest],M):-
	!,
	update_feature_weight(F,M),
	winnow_multiply_weight(Rest,M).
winnow_multiply_weight([_|Rest],M):-
	winnow_multiply_weight(Rest,M).
...
update_besthyp(Num):-
	...
	asserta('$aleph_global'(besthyp,besthyp(Label,Num,H,PCover,NCover))), !.
update_besthyp(_).


% set restrictions on search for features
set_feature_search_restrictions(MaxNodes,MaxFeatures):-
	get_example_class(Class),
	(setting(streaming,true) ->
		FLeft is MaxFeatures;
		('$aleph_feature_count'(Class,Last) ->
			(Last >= MaxFeatures -> FLeft is 0;
				FLeft is MaxFeatures - Last);
			FLeft is MaxFeatures)
	),
        ('$aleph_search'(last_good,LastGood) ->
		MaxGood is LastGood + FLeft;
		MaxGood is FLeft),
	retractall('$aleph_search'(max_features,_)),
	asserta('$aleph_search'(max_features,FLeft)),
	retractall('$aleph_search'(max_good,_)),
	asserta('$aleph_search'(max_good,MaxGood)),
	(FLeft = 0  ->
		set(nodes,1);
		set(nodes,MaxNodes)), !.
set_feature_search_restrictions(MaxNodes,MaxFeatures):-
	set(nodes,MaxNodes),
        ('$aleph_search'(last_good,LastGood) ->
		MaxGood is LastGood + MaxFeatures;
		MaxGood is MaxFeatures),
	retractall('$aleph_search'(max_features,_)),
	asserta('$aleph_search'(max_features,MaxFeatures)),
	retractall('$aleph_search'(max_good,_)),
	asserta('$aleph_search'(max_good,MaxGood)).


update_feature_count(Class,F):-
	retract('$aleph_feature_count'(Class,Last)),
	Last1 is Last + F,
	asserta('$aleph_feature_count'(Class,Last1)), !.
update_feature_count(Class,F):-
	asserta('$aleph_feature_count'(Class,F)).

get_example_class(Class):-
	'$aleph_global'(example_selected,example_selected(pos,N)),
	'$aleph_example'(N,pos,Example),
	setting(dependent,ClassArg),
	arg(ClassArg,Example,Class), !.
get_example_class(pos).


% generate a new feature from a good clause
gen_features:-
	clean_up_features,
	(setting(minscore,FMin) -> true; number(-inf,FMin)),
	'$aleph_good'(_,_,Label,_,_,Clause),
	Label = [_,_,_,F|_],
	F >= FMin,
	gen_feature(Clause,Label),
	fail.
gen_features:-
	...
	repeat,
	read(Stream,Fact),
	(Fact = '$aleph_good'(_,_,Label,_,_,Clause) ->
		Label = [_,_,_,F|_],
		F >= FMin,
		gen_feature(Clause,Label),
		fail;
...
gen_feature_from_good_clause(GoodId):-
	(setting(minscore,FMin) -> true; number(-inf,FMin)),
	'$aleph_good'(_,GoodId,Label,_,_,Clause),
	Label = [_,_,_,F|_],
	F >= FMin,
	gen_feature(Clause,Label), !.
gen_feature_from_good_clause(_).
...
get_performance:-
	...
	fail.
get_performance:-
	(setting(valid_pos,PFile) ->
		test(PFile,noshow,Tp,TotPos),
		Fn is TotPos - Tp;
		TotPos = 0, Tp = 0, Fn = 0),
	(setting(valid_neg,NFile) ->
		test(NFile,noshow,Fp,TotNeg),
		Tn is TotNeg - Fp;
		TotNeg = 0, Tn = 0, Fp = 0),
	TotPos + TotNeg > 0,
	p_message('Validation set performance'),
	write_cmatrix([Tp,Fp,Fn,Tn]),
	p1_message('Validation set summary'), p_message([Tp,Fp,Fn,Tn]),
	fail.
get_performance:-
...
lazy_evaluate_theory([Refine|T],LazyPreds,Pos,Neg,[Refine1|T1]):-
	(Refine = A-[B,C,D,Clause]->
		...
		lazy_evaluate_refinement([],_Clause,LazyPreds,Pos,Neg,_D1,Refine1)),
...
lazy_evaluate1(false,Atom,_,I,O,C,_,Lit,NewLits):-
	functor(Atom,Name,Arity),
	p1_message(1,'lazy evaluation'), p_message(1,Name),
...
lazy_index_prove1(Type,Lit,Head,Body,Num):-
        depth_bound_call(('$aleph_example'(Num,Type,Head),Body)),
...
gsample(Name/Arity,N):-
	...
	N2 is N1 + 1,
	asserta('$aleph_local'(slp_samplenum,N2)),
	assertz('$aleph_example'(N2,rand,Pred)),
	N2 >= N,
	!,
	retract('$aleph_local'(slp_samplenum,N2)),
...
record_clause(good,Label,Clause,_):-
	...
	('$aleph_sat'(example,example(Num,Type)) -> true;
		Num = 0, Type = pos),
	aleph_writeq('$aleph_good'(L,_,Label,Num,Type,Clause)),  write('.'), nl,
	flush_output(Stream),
	set_output(user_output).
...
record_sat_example(N):-
	setting(recordfile_stream,Stream), !,
	set_output(Stream),
	p1_message(1,'sat'), p_message(1,N),
	flush_output(Stream),
	set_output(user_output).
...
show_total_stats:-
	p_message(1,'search statistics'),
	show_total_search_stats,
	show_other_stats, !.
show_total_stats.

show_total_search_stats:-
	'$aleph_global'(search_stats,search_stats(Nodes,_)),
	p1_message(1,'total clauses constructed'), p_message(1,Nodes), !.
show_total_search_stats.


show_other_stats:-
	'$aleph_global'(other_stats,X),
	p_message(1,X),
	fail.
show_other_stats.
...
bvar_types([not(Lit)|Lits],VTSoFar,BVarTypes):-
        bvar_types([Lit|Lits],VTSoFar,BVarTypes).
...
tautology((false:-Body)):-
	!,
	aleph_in(Body,L1,Rest),
	aleph_in(Rest,not(L2)),
	L1 == L2.
tautology((Head:-Body)):-
	aleph_in(Body,Lit),
	Head == Lit, !.
...
get_type_elements([Pred|Preds]):-
	...
	get_type_elements('$aleph_example'(_,_,Template),Template,Pred,AL),
...
estimate_clauselength_distribution(L,T,K,D):-
	'$aleph_sat'(example,example(Example,Type)),
...
estimate_numbers(Total):-
	(('$aleph_sat'(example,example(N,_)), N > 0) -> true; rsat), !,
	setting(clauselength,CL),
	estimate_numbers(CL,1,400,Total).
estimate_numbers(inf).
...
estimate_numbers(L,Trials,Sample,Total):-
	'$aleph_sat'(example,example(Example,Type)),
	'$aleph_sat'(random,sample(Example,Type,L,Trials,Sample)),
	'$aleph_sat'(random,hypothesis_space(Total)), !.
estimate_numbers(L,Trials,Sample,Total):-
	'$aleph_sat'(example,example(Example,Type)),
	get_bottom_key(Example,Type,Key),
	Key \= false,
	'$aleph_sat'(random,Key,sample(Example,Type,L,Trials,Sample)),
	'$aleph_sat'(random,Key,hypothesis_space(Total)), !.
estimate_numbers(L,Trials,Sample,Total):-
	...
	'$aleph_sat'(example,example(Example,Type)),
	asserta('$aleph_sat'(random,sample(Example,Type,L,Trials,Sample))).
...
randclause(L,C,Status,Lits):-
	...
	clause_status(Lits,Key,[],Status1),
	Status1 = Status, !,
        (Key = false ->
		get_pclause(Lits,[],C,_,_,_);
		get_pclause(Lits,Key,[],C,_,_,_)).

% clause_status(+Lits,+LitsSoFar,+Flag,-Status)
% compute status of a clause
%	Lits is the lits left to add to the clause
%	LitsSoFar is the lits in the clause so far
clause_status(Lits,LitsSoFar,Status):-
	get_bottom_key(_,_,Key),
	clause_status(Lits,Key,LitsSoFar,Status).

clause_status([],_,_,legal):- !.
clause_status([Lit|Lits],Key,LitsSoFar,S1):-
	get_ovars(LitsSoFar,Key,[],OVars),
	get_ivars([Lit],Key,[],IVars),
	aleph_subset1(IVars,OVars), !,
	aleph_append([Lit],LitsSoFar,Lits1),
	clause_status(Lits,Key,Lits1,S1).
clause_status(_,_,_,illegal).
...
clause_move(delete_lit,C1,C2):-
	...
	(Lits = [H|Rest] ->
		...
		Lits1 = [H|Left],
		get_bottom_key(E,T,Key),
		clause_status(Lits1,Key,[],false,legal),
		L1 is L - 1,
		(Key = false ->
...
old_move(theories,T):-
	...
	numbervars(T,0,_),
	length(T,N),
	('$aleph_search_seen'(N,_Hash) ->
		p_message('old move'),
		true;
		asserta('$aleph_search_seen'(N,_Hash)), !,
		fail).
...
pp_dclause(Clause):-
		asserta('$pycuity_search_store'(Clause)),
...
update_exact([],H,[H]).
update_exact([H|T],H1,[H|T]):- H == H1, !.
update_exact([H1|T],H,[H1|T1]):-
	update_exact(T,H,T1).
...
% inefficient union of intervals
intervals_union(Intervals1,Intervals2,Intervals):-
	intervals_to_list(Intervals1,L1),
	intervals_to_list(Intervals2,L2),
	aleph_append(L1,L2,L),
	list_to_intervals(L,Intervals).
...
clean_up_search:-
	...
	retractall('$aleph_search'(selected,_)),
	retractall('$aleph_search_seen'(_,_)),
	retractall('$aleph_search_node'(_,_)),
...
clean_up_features:-
	...
	retractall('$aleph_feature_count'(_,_)),
	retractall('$aleph_feature_weight'(_,_)),
	retractall('$aleph_feature'(_,_,_,_,_)).
...
clean_up_examples:-
	...
	retractall('$aleph_feature_vector'(_,_,_)),
	aleph_abolish('$aleph_example'/3),
	assert('$aleph_example'(0,uspec,false)).
...
clean_up_examples(Type):-
	...
	asserta('$aleph_global'(atoms,atoms(Type,[]))),
	asserta('$aleph_global'(atoms_left,atoms_left(Type,[]))),
	asserta('$aleph_global'(last_example,last_example(Type,0))).
...
record_example(check,Type,Example,N1):-
	check_example_type(Example,Type),
	(once('$aleph_example'(N1,Type,Example)) -> true;
...
check_example_type(_,pos):-
	setting(streaming,true), !.
check_example_type(E,Type):-
	var(Type), !,
	get_example_class(E,Class),
	((Class = neg ; Class = pos) -> Type = Class; Type = pos).
check_example_type(_,_).

get_example_class((:- _),neg):- !.
get_example_class(Example,Class):-
	setting(dependent,ClassArg), !,
	arg(ClassArg,Example,Class).
get_example_class(_,pos).
...
check_auto_refine:-
	...
	setting(refineop,Refine),
	((Refine = user; Refine = rls) -> true; set(refine,auto)).
check_auto_refine.
...
get_validation_set(Valid):-
	get_validation_set(Valid,pos),
	get_validation_set(Valid,neg).

get_validation_set(Valid,Type):-
        '$aleph_global'(size,size(Type,S)),
	ValidSize is integer(Valid*S),
	retractall('$aleph_global'(validation,example(Type,_))),
	get_validation_set(ValidSize,Type,S).

get_validation_set(0,_,_):- !.
get_validation_set(N,Type,Last):-
	Condition = '$aleph_global'(validation,example(Type,Num)),
	get_random(10,Last,not(Condition),Num),
	assertz('$aleph_global'(validation,example(Type,Num))),
	N1 is N - 1,
	get_validation_set(N1,Type,Last).
...
record_nskolemized(Type,N1,Head,true):-
	!,
	assertz('$aleph_example'(N1,Type,Head)).
record_nskolemized(Type,N1,Head,Body):-
	assertz(('$aleph_example'(N1,Type,Head):-Body)).
...
reset_counts:-
	...
	retractall('$aleph_local'(botsize,_)),
	asserta('$aleph_local'(botsize,0)),
...
gen_featurenum(Feature1):-
        '$aleph_feature_count'(Feature0), !,
        Feature1 is Feature0 + 1,
        retract('$aleph_feature_count'(Feature0)),
        asserta('$aleph_feature_count'(Feature1)).
gen_featurenum(Feature1):-
	setting(feature_offset,Offset),
...
add_good_clauses(Type,Num):-
	setting(good,true),
	setting(add_good,true), !,
	'$aleph_global'(last_clause,last_clause(ClauseIndex)),
	'$aleph_global'(theory,theory(ClauseIndex,Label/Num,_,_,_)),
	Label = [_,_,_,F|_],
	add_good_clauses(F,Type,Num).
add_good_clauses(_,_).

add_good_clauses(F,Type,Num):-
	store(lazy_on_cost),
	set(lazy_on_cost,false),
	'$aleph_good'(L,_,_,Num,Type,Clause),
	...
	NewLabel = [_,_,_,F1|_],
	F1 = F,
        asserta('$aleph_global'(hypothesis,hypothesis(NewLabel,Clause,
				PCover,NCover))),
	once(update_theory(_)),
	fail.
add_good_clauses(_,_,_):-
	reinstate(lazy_on_cost).
...
update_theory(ClauseIndex):-
	...
	index_clause(Hypothesis,ClauseIndex,Clause),
        ('$aleph_global'(example_selected,example_selected(_Type,Seed))-> true;
                PCover = [Seed-_|_]),
	(setting(lazy_on_cost,true) ->
		...
		setting(evalfn,Evalfn),
		complete_label(Evalfn,Clause,[PC,NC,L],NewLabel),
                add_clause_to_theory(ClauseIndex,NewLabel/Seed,Clause,PCover,NCover);
                add_clause_to_theory(ClauseIndex,OldLabel/Seed,Clause,OldPCover,OldNCover)).

add_clause_to_theory(ClauseIndex,Label/Seed,Clause,PCover,NCover):-
        assertz('$aleph_global'(theory,theory(ClauseIndex, Label/Seed,Clause, PCover,NCover))),
...
add_clause_to_theory(_,_,_,_,_).

add_clause_to_background(ClauseIndex):-
	setting(construct_features,false),
...
rm_seeds:-
	...
	p1_message(1,'atoms left'), p_message(1,PL),
	!.
rm_seeds.
...
gen_sample(Type,_):-
	'$aleph_global'(atoms_left,atoms_left(Type,[])), !,
	retractall('$aleph_global'(example_selected,example_selected(_,_))),
	(setting(resample,Resample) -> true; Resample = 1),
	gen_sample(Resample,Type,0).
gen_sample(Type,0):-
	!,
...
get_rrandom(Last,Num):-
	...
	Num is X*Last.
%
% get_random(+N,+Last,+Condition,-Num)
%	make N attempts to get a random integer between 1 and Last
%	such that Condition is true.
get_random(0,_,_,_):- !, fail.
get_random(_,Last,Condition,Num):-
	get_random(Last,Num),
	call(Condition), !.
get_random(N,Last,Condition,Num):-
	N1 is N - 1,
	get_random(N1,Last,Condition,Num).
...
label_create(Clause,ExtendedLabel):-
	...
	(setting(evalfn,posonly) ->
		...
		Type2 = neg),
	label_create(Clause,Type1,[1-Last1],Type2,[1-Last2],ExtendedLabel).

label_create(Type,Clause,ExtendedLabel):-
        '$aleph_global'(last_example,last_example(Type,Last)),
	label_create(Clause,Type,[1-Last],ExtendedLabel).

label_create(Clause,Type1,Set1,Type2,Set2,ExtendedLabel):-
        split_clause(Clause,Head,Body),
	nlits((Head,Body),Length),
	('$aleph_search'(pclause,pclause(Head,Body))->
		ClauseExists = true; ClauseExists = false),
        (ClauseExists = false ->
		assertz('$aleph_search'(pclause,pclause(Head,Body)));
		true),
	setting(depth,Depth),
	...
	(ClauseExists = false ->
		retractall('$aleph_search'(pclause,_));
		true),
        assemble_label(Cover1,Cover2,Length,ExtendedLabel), !.

label_create(Clause,Type,Set,ExtendedLabel):-
        split_clause(Clause,Head,Body),
	('$aleph_search'(pclause,pclause(Head,Body))->
		ClauseExists = true; ClauseExists = false),
        (ClauseExists = false ->
	...
	(ClauseExists = false ->
		...
		true),
	(Type = pos ->
		assemble_label(Cover,unknown,unknown,ExtendedLabel);
		assemble_label(unknown,Cover,unknown,ExtendedLabel)).


label_cover(pos,[P|_],P).
label_cover(neg,[_,N|_],N).
...
evalfn(mestimate,[P,N|_],Val):-
		...
		aleph_member1(Prior-pos,NPriorD),
                ...
                Val is (P + K*Prior) / (Cover+K)), !.
evalfn(_,_,X):- number(-inf,X).


assemble_label(P,N,L,[P,N,L]).

assemble_label(ExtendedLabel,Label):-
	extract_counts(ExtendedLabel,Label).
...
extract_pos_count([P|_],P):- number(P), !.
extract_pos_count(ExtendedLabel,P):- extract_count(pos,ExtendedLabel,P).

extract_neg_count([_,N|_],N):- number(N), !.
extract_neg_count(ExtendedLabel,N):-  extract_count(neg,ExtendedLabel,N).

extract_pos([P|_],P).
extract_neg([_,N|_],N).
extract_length([_,_,L|_],L).
extract_eval([_,_,_,F|_],F).

extract_counts(ExtendedLabel,[P,N,L]):-
	extract_count(pos,ExtendedLabel,P),
	extract_count(neg,ExtendedLabel,N),
	extract_length(ExtendedLabel,L).

get_start_label(_,[0,0,0,F]):-
	(setting(interactive,true);
		setting(search,ic);
		setting(proof_strategy,user)), !,
	number(-inf,F).
get_start_label(_,[0,0,0,F]):-
	'$aleph_global'(size,size(pos,0)),
	'$aleph_global'(size,size(neg,0)), !,
	number(-inf,F).
get_start_label(user,[1,0,2,F]):- !, number(-inf,F).
...
read_examples_files(Type,Name,F):-
	...
	(N > 0 -> Ex = [1-N]; Ex = []),
	retract('$aleph_global'(atoms,atoms(Type,_))),
	asserta('$aleph_global'(atoms,atoms(Type,Ex))),
	retract('$aleph_global'(atoms_left,atoms_left(Type,_))),
...
store_bottom:-
	get_bottom_key(Num,Type,false),
	set_bottom_key(Num,Type,Key),
...
% get_bottom_key(?N,?T,-Key)
%	returns key that indexes bottom clause info for example N of type T
%	if no stored bottom clause for N and T, then Key is "false"
get_bottom_key(N,T,Key):-
	((var(N),var(T)) ->
		'$aleph_sat'(example,example(N,T));
		true),
	('$aleph_sat'(stored,stored(N,T,Key)) -> true; Key = false).
% set_bottom_key(?N,?T,-Key)
%	returns key that indexes bottom clause info for example N of type T
set_bottom_key(N,T,Key):-
	((var(N),var(T)) ->
		'$aleph_sat'(example,example(N,T));
...
model(Name/Arity):-
        assertz('$aleph_global'(model_stats,model(Name/Arity))).
...
feature(Id,Feature):-
	'$aleph_feature'(Id,_,_,Template,Body),
	Feature = (Template:-Body).

feature_vector(Example,FeatureVector):-
	get_feature_vector(Example,FeatureVector).

store_feature_vector(Example,FeatureVector):-
	'$aleph_example'(Id,Type,Example),
	asserta('$aleph_feature_vector'(Id,Type,FeatureVector)).

feature_vector(Id,Type,FeatureVector):-
	'$aleph_feature_vector'(Id,Type,FeatureVector), !.
feature_vector(Id,Type,FeatureVector):-
	(setting(dependent,Arg)->
		'$aleph_example'(Id,pos,Example),
		arg(Arg,Example,Type);
		'$aleph_example'(Id,Type,Example)),
	get_feature_vector(Example,FeatureVector),
	('$aleph_example'(Id,Type,Example) ->
		asserta('$aleph_feature_vector'(Id,Type,FeatureVector));
...
get_feature_vector(Example,FeatureVector):-
	findall(F-Value,(feature(F,(Example:-Body)),
			(once(Body)->Value=1;Value=0)),
	FeatureVector).

% feature_vector(Example,FeatureVector):-
	% last_feature(N),
	% feature_vector(1,N,Example,FeatureVector).
%
% feature_vector(F1,F,_,[]):- F1 > F, !.
% feature_vector(F1,F,Example,[F1-1|Rest]):-
	% '$aleph_feature'(F1,_,_,Template,Body),
	% \+ feature_unsatisfied(Template,Body,Example), !,
	% F2 is F1 + 1,
	% feature_vector(F2,F,Example,Rest).
% feature_vector(F1,F,Example,[F1-0|Rest]):-
	% F2 is F1 + 1,
	% feature_vector(F2,F,Example,Rest).
%
% feature_unsatisfied(Example,Body,Example):-
	% once(Body), !, fail.
% feature_unsatisfied(_,_,_).

feature_weight(F,W):-
	'$aleph_feature_weight'(F,W), !.
feature_weight(_,1.0).

last_feature(N):-
	'$aleph_feature_count'(N), !.
last_feature(0).
...
gen_feature(Feature,Label,Class):-
	...
	assertz('$aleph_feature'(Id,Label,Class,Template,Body)),
...
show(features):-
        % setting(evalfn,Evalfn),
	('$aleph_feature'(_,_,_,_,_) -> true;
		gen_features),
        p_message('features from good clauses'),
	'$aleph_feature'(Id,Label,X,Head,Body),
	% show_stats(Evalfn,Label),
	pp_dclause('$aleph_feature'(Id,Label,X,Head,Body)),
	fail.
show(features):-
	'$aleph_feature_weight'(Id,W),
	pp_dclause('$aleph_feature_weight'(Id,W)),
	fail.
show(features):-
	'$aleph_feature_count'(N),
	pp_dclause('$aleph_feature_count'(N)),
	fail.
show(aleph_features):-
...
show(model):-
	setting(verbosity,Verbosity),
	Verbosity > 0,
	setting(model_type,Model),
	concat([Model,' ', model],ModelName),
	p_message(ModelName),
	show(Model).
show(constraints):-
	setting(good,true),
...
show(Name/Arity):-
	...
	clause(Pred,Body),
	\+(aleph_in(Body,'$aleph_search'(pclause,pclause(_,_)))),
...
show(winnow2):-
	show(winnow).
show(winnow):-
        p1_message('Feature No.'), tab(4), p1_message('Weight'), tab(4),
	p_message('Feature'), nl, nl,
	findall(MW-F,(feature(F,_),feature_weight(F,W),MW is -W),WFL),
	keysort(WFL,WFLs),
	aleph_member(MW-Id,WFLs),
	'$aleph_feature'(Id,_,_,Head,Body),
	Weight is -MW,
	write(Id), tab(4),
	write(Weight), tab(8),
        numbervars((Head:-Body),0,_),
...
% examples(?Type,?List)
% if List is a variable, and Type is given return list of examples of type Type
% otherwise
% show all examples numbers in List of Type
examples(Type,List):-
	nonvar(Type),
	var(List),
	get_examples(Type,List).
examples(Type,List):-
	setting(portray_literals,Pretty),
        '$aleph_example'(Num,Type,Atom),
...
get_examples(Type,List):-
	setting(dependent,Class), !,
	findall(Num,('$aleph_example'(Num,pos,Example),
			arg(Class,Example,Type)),List).
get_examples(Type,List):-
	'$aleph_global'(atoms,atoms(Type,Intervals)),
	intervals_to_list(Intervals,List).

example(Id,Type,Example):-
	'$aleph_example'(Id,Type,Example).
...
write_features(File):-
        aleph_open(File,write,Stream),
        set_output(Stream),
	write_aleph_features,
        close(Stream),
        set_output(user_output).
write_features(_).

write_aleph_features:-
	'$aleph_feature'(A,B,C,D,E),
	writeq('$aleph_feature'(A,B,C,D,E)), write('.'), nl,
	fail.
write_aleph_features:-
	'$aleph_feature_weight'(A,B),
	writeq('$aleph_feature_weight'(A,B)), write('.'), nl,
	fail.
write_aleph_features:-
	'$aleph_feature_count'(A),
	writeq('$aleph_feature_count'(A)), write('.'), nl, !.
write_aleph_features.
...
coversn(N):-
	...
	asserta('$aleph_search'(coversn,coversn(NCover,N))).
...
aleph_in((Head:-true),Head):- !.
aleph_in((Head:-Body),L):-
	!,
	aleph_in((Head,Body),L).
aleph_in((L1,_),L1).
aleph_in((_,R),L):-
...
set_def(check_good, miscellaneous,
	...
	[true, false], false,
	show).
set_def(check_good_dist, miscellaneous,
	'Distance between good clauses for duplicates',
...
set_def(clauses, search-search_space,
	...
	int(1)-int(+inf),'',
	show).
set_def(clausetype, search-search_space,
	'Constraint on clause type (usually for features)',
	[elementary,simple,rsd,independent,any],any,
	show).
...
set_def(explore, search-search_space,
	...
	[true, false], false,
	show).
set_def(feature_construction, search-search_space,
	'Feature construction method',
	[greedy, exhaustive, random, theory, user], exhaustive,
	show).
set_def(feature_offset, miscellaneous,
	'Offset for first feature number',
	int(0)-int(+inf), 0,
	show).
...
set_def(i, saturation,
	...
	int(1)-int(+inf), 2,
	show).
set_def(induce,search-search_strategy,
	'Hypothesis construction method',
	[greedy,max,cover,incremental,stream,tree,constraints,theory,model,modes],greedy,
	show).
...
set_def(language, search-search_space,
	'Maximum occurrence of a predicate symbol in a clause',
	int(1)-int(+inf),+inf,
	show).
...
set_def(max_features, miscellaneous,
	...
	int(1)-int(+inf), +inf,
	show).
set_def(max_botsize, saturation,
        'Maximum number of literals in a bottom clause',
        int(1)-int(+inf), +inf,
        show).
...
set_def(stage, miscellaneous,
	...
	[saturation, reduction, theory_reduction, command], command,
	show-ro).
...
set_def(test_pos, miscellaneous,
	...
	read(filename),'',
	show).
set_def(valid_pos, miscellaneous,
	'Positive examples for validating theory',
	read(filename),'',
	show).
set_def(valid_neg, miscellaneous,
	'Negative examples for validating theory',
	read(filename),'',
	show).
...
special_consideration(search,scs):-
	set(refine,rls), !.
...
special_consideration(minscore,_):-
	clean_up_features.
special_consideration(clausetype,_):-
	retractall('$aleph_sat'(random,sample(_,_,_,_,_))),
	retractall('$aleph_sat'(random,hypothesis_space(_))), !.
special_consideration(stream,S):-
	set(test_pos,S), !.
...
rm_special_consideration(stream,S):-
	close(S),
	noset(test_pos), !.
...
get_theory(Theory):-
        '$aleph_global'(rules,rules(L)),
        aleph_reverse(L,L1),
	findall(Clause,(aleph_member(ClauseNum,L1),
			'$aleph_global'(theory,theory(ClauseNum,_,Clause,_,_))),
		Theory).
...
get_hyp_label((_:-Body),[P,N,L]):-
        ...
        ('$aleph_search'(coversn,coversn(_,N))-> true;
                        coversn(_),
