...
def generate_trajectories(n, world, policy, initial, terminal):
    """
    ...
    """
    start_states = np.atleast_1d(initial)
...
def clipping_policy_adapter(policy):
    return lambda state: np.argmax(policy[state, :])
